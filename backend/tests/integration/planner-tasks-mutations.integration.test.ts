...
test('Planner Tasks Mutations API Integration Suite', async (t) => {
  ...
  const dPlus5 = getTestDate(5);
  const dPlus7 = getTestDate(7);
  const dPlus10 = getTestDate(10);
  const dPlus11 = getTestDate(11);
  ...
  await t.test('Scheduled Tasks Complete + Undo', async (sub) => {
    await sub.test('completes an ONCE task, idempotently, and undoes it', async () => {
      ...
      const createRes = await fetch(`${baseUrl}/tasks`, {
        method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token1}` },
        body: JSON.stringify({ id: taskId, title: 'Once Task', plannerToday: today, schedule: { type: 'ONCE', startDate: today } })
      });
      const createBody = await createRes.json();
      const scheduleId = createBody.task.schedules[0].id;

      const completePayload = { plannerToday: today, completedDate: today, scheduleId, scheduledDate: today };
    ...
    await sub.test('rejects a completion on a date not generated by the schedule', async () => {
      const taskId = getValidId();
      const dNow = new Date();
      const currentWeekday = dNow.getUTCDay() === 0 ? 7 : dNow.getUTCDay();
      const nonMatchingWeekday = (currentWeekday % 7) + 1;
      const nonMatchingMask = 1 << (nonMatchingWeekday - 1);

      const createRes = await fetch(`${baseUrl}/tasks`, {
        method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token1}` },
        body: JSON.stringify({ id: taskId, title: 'Weekly Task', plannerToday: today, schedule: { type: 'WEEKDAYS', weekdaysMask: nonMatchingMask, startDate: today } })
      });
      const createBody = await createRes.json();
      const scheduleId = createBody.task.schedules[0].id;
      ...
      const res = await fetch(`${baseUrl}/tasks/${taskId}/complete`, {
        method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token1}` },
        body: JSON.stringify({ plannerToday: today, completedDate: today, scheduleId, scheduledDate: today })
      });
  ...
  await t.test('Delete Task', async (sub) => {
    await sub.test('hard deletes task and cascades', async () => {
      ...
      await fetch(`${baseUrl}/tasks`, {
        method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token1}` },
        body: JSON.stringify({ id: taskId, title: 'To Delete', plannerToday: today, schedule: { type: 'ONCE', startDate: today } })
      });
  ...
  await t.test('Stop Recurrence', async (sub) => {
    await sub.test('removes future empty segments and ends current active segment without invalidating past completions', async () => {
      ...
      const createRes = await fetch(`${baseUrl}/tasks`, {
        method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token1}` },
        body: JSON.stringify({ id: taskId, title: 'Recurring', plannerToday: yesterday, schedule: { type: 'INTERVAL_DAYS', intervalDays: 1, startDate: yesterday } })
      });
      const createBody = await createRes.json();
      const scheduleId = createBody.task.schedules[0].id;
      ...
      await fetch(`${baseUrl}/tasks/${taskId}/complete`, {
        method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token1}` },
        body: JSON.stringify({ plannerToday: today, completedDate: yesterday, scheduleId, scheduledDate: yesterday })
      });
      
      const res = await fetch(`${baseUrl}/tasks/${taskId}/stop-recurrence`, {
        method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token1}` },
        body: JSON.stringify({ plannerToday: today })
      });
      assert.equal(res.status, 200);
      
      const schedCheck = await pool.query('SELECT end_date::text FROM task_schedules WHERE id = $1', [scheduleId]);
      assert.equal(schedCheck.rows[0].end_date, today);
    });
    ...
    await sub.test('hard deletes task if it never generated any occurrence and has no completions', async () => {
      ...
      const createRes = await fetch(`${baseUrl}/tasks`, {
        method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token1}` },
        body: JSON.stringify({ id: taskId, title: 'Never generated', plannerToday: today, schedule: { type: 'INTERVAL_DAYS', intervalDays: 1, startDate: dPlus5 } })
      });
    ...
    await sub.test('rejects stop recurrence if a future completion prevents it safely', async () => {
      ...
      const createRes = await fetch(`${baseUrl}/tasks`, {
        method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token1}` },
        body: JSON.stringify({ id: taskId, title: 'Future Completion', plannerToday: today, schedule: { type: 'INTERVAL_DAYS', intervalDays: 1, startDate: today } })
      });
      const createBody = await createRes.json();
      const scheduleId = createBody.task.schedules[0].id;
      
      // Complete it on dPlus5 (future occurrence relative to plannerToday)
      await fetch(`${baseUrl}/tasks/${taskId}/complete`, {
        method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token1}` },
        body: JSON.stringify({ plannerToday: today, completedDate: today, scheduleId, scheduledDate: dPlus5 })
      });

      // Manually set scheduled_date completion in db to test future completion guard
      await pool.query('UPDATE task_completions SET scheduled_date = $1 WHERE task_id = $2', [dPlus5, taskId]);

      // Attempt to stop recurrence on today
      const res = await fetch(`${baseUrl}/tasks/${taskId}/stop-recurrence`, {
        method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token1}` },
        body: JSON.stringify({ plannerToday: today })
      });
    ...
    await sub.test('rejects stop recurrence if a future segment has recorded completions', async () => {
      ...
      const createRes = await fetch(`${baseUrl}/tasks`, {
        method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token1}` },
        body: JSON.stringify({ id: taskId, title: 'Future Segment', plannerToday: yesterday, schedule: { type: 'INTERVAL_DAYS', intervalDays: 1, startDate: yesterday } })
      });
      const createBody = await createRes.json();

      // Manually add a future segment with a completion
      const futureSchedRes = await pool.query(
        `INSERT INTO task_schedules (task_id, schedule_type, start_date, end_date, interval_days, interval_anchor_date) VALUES ($1, 'INTERVAL_DAYS', $2, $3, 1, $2) RETURNING id`,
        [taskId, dPlus5, dPlus10]
      );
      const schedule2Id = futureSchedRes.rows[0].id;
      await pool.query(
        `INSERT INTO task_completions (task_id, schedule_id, scheduled_date, completed_date, completed_at, title_snapshot, is_important_snapshot) VALUES ($1, $2, $3, $4, NOW(), 'Future Segment', false)`,
        [taskId, schedule2Id, dPlus5, today]
      );
    ...
    await sub.test('stops finite currently-effective recurring segment at plannerToday', async () => {
      ...
      const createRes = await fetch(`${baseUrl}/tasks`, {
        ...
        body: JSON.stringify({
          ...
          schedule: { type: 'INTERVAL_DAYS', intervalDays: 1, startDate: yesterday, endDate: dPlus11 }
        })
      });
      const createBody = await createRes.json();
      const scheduleId = createBody.task.schedules[0].id;
      ...
      // Attempting completion after today should now fail with 409 Conflict
      const compRes = await fetch(`${baseUrl}/tasks/${taskId}/complete`, {
        method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token1}` },
        body: JSON.stringify({ plannerToday: today, completedDate: today, scheduleId, scheduledDate: tomorrow })
      });
    ...
    await sub.test('preserves task and historical schedule when removing a never-generated future segment', async () => {
      ...
      const createRes = await fetch(`${baseUrl}/tasks`, {
        ...
        body: JSON.stringify({
          ...
          plannerToday: yesterday,
          schedule: { type: 'INTERVAL_DAYS', intervalDays: 1, startDate: yesterday, endDate: yesterday }
        })
      });
      const createBody = await createRes.json();
      const histScheduleId = createBody.task.schedules[0].id;

      // Add a future recurring segment that will never generate an occurrence before today
      const futureSchedRes = await pool.query(
        `INSERT INTO task_schedules (task_id, schedule_type, start_date, end_date, interval_days, interval_anchor_date) VALUES ($1, 'INTERVAL_DAYS', $2, $3, 1, $2) RETURNING id`,
        [taskId, dPlus5, dPlus11]
      );
  ...
  await t.test('Concurrency and Rollback Proofs', async (sub) => {
    ...
    await sub.test('mid-transaction PostgreSQL failure rolls back prior mutations', async () => {
      ...
      try {
        ...
        await client.query(`INSERT INTO task_completions (task_id, completed_date) VALUES ('not-a-uuid', $1)`, [today]);
  ...
  await t.test('Completion Date Business Integrity Validation', async (sub) => {
    await sub.test(`1-5: past occurrence completedDate boundary rules (scheduledDate=${dMinus4}, plannerToday=${today})`, async () => {
      const taskId = getValidId();
      const createRes = await fetch(`${baseUrl}/tasks`, {
        method: 'POST',
      ...
      await fetch(`${baseUrl}/tasks/${taskId}/undo`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token1}` },
        body: JSON.stringify({ scheduleId, scheduledDate: dMinus4 })
      });
    ...
    await sub.test(`6-7: future occurrence completion rules (scheduledDate=${dPlus7}, plannerToday=${today})`, async () => {
      const taskId = getValidId();
      const createRes = await fetch(`${baseUrl}/tasks`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token1}` },
        body: JSON.stringify({
          ...
          title: 'Future Task',
          plannerToday: today,
          schedule: { type: 'ONCE', startDate: dPlus7 }
        })
      });
      assert.equal(createRes.status, 201);
      const scheduleId = (await createRes.json()).task.schedules[0].id;
    ...
    await sub.test(`8-10: Later task completion rules (plannerToday=${today})`, async () => {
      const taskId = getValidId();
      await fetch(`${baseUrl}/tasks`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token1}` },
      ...
      const resHist = await fetch(`${baseUrl}/tasks/${taskId}/complete`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token1}` },
        body: JSON.stringify({ plannerToday: today, completedDate: yesterday })
      });
      ...
      const resFut = await fetch(`${baseUrl}/tasks/${taskId}/complete`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token1}` },
        body: JSON.stringify({ plannerToday: today, completedDate: tomorrow })
      });
    ...
    await sub.test('11: missing or malformed plannerToday fails with 400', async () => {
      ...
      const resMalformed = await fetch(`${baseUrl}/tasks/${taskId}/complete`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token1}` },
        body: JSON.stringify({ plannerToday: 'not-a-date', completedDate: today })
      });
    ...
    await sub.test('POST /tasks/:taskId/stop-recurrence rejects implausible plannerToday spoofing', async () => {
      ...
      await fetch(`${baseUrl}/tasks`, {
        ...
        body: JSON.stringify({
          ...
          plannerToday: today,
          schedule: { type: 'INTERVAL_DAYS', intervalDays: 1, startDate: today }
        })
      });
  ...
  await t.test('Adversarial Authorization, Malformed Input & Boundary Regression Tests', async (sub) => {
    await sub.test('User A uses User B scheduleId against User A task completion is rejected safely', async () => {
      ...
      const createARes = await fetch(`${baseUrl}/tasks`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token1}` },
        body: JSON.stringify({ id: taskAId, title: 'Task A', plannerToday: today, schedule: { type: 'ONCE', startDate: today } })
      });
      ...
      const createBRes = await fetch(`${baseUrl}/tasks`, {
        ...
        body: JSON.stringify({ id: taskBId, title: 'Task B', plannerToday: today, schedule: { type: 'ONCE', startDate: today } })
      });
    ...
    await sub.test('duplicate plannerToday query parameters on History returns controlled 400', async () => {
      const res = await fetch(`${baseUrl}/planner/history?plannerToday=${today}&plannerToday=${tomorrow}`, {
        headers: { 'Authorization': `Bearer ${token1}` }
      });
      assert.equal(res.status, 400);
    });
    ...
    await sub.test('GET /planner/history with q exactly 200 chars is accepted', async () => {
      ...
      const res = await fetch(`${baseUrl}/planner/history?plannerToday=${today}&q=${q200}`, {
        headers: { 'Authorization': `Bearer ${token1}` }
      });
      assert.equal(res.status, 200);
    });
    ...
    await sub.test('GET /planner/history with q 201 chars is rejected with 400', async () => {
      ...
      const res = await fetch(`${baseUrl}/planner/history?plannerToday=${today}&q=${q201}`, {
        headers: { 'Authorization': `Bearer ${token1}` }
      });
      assert.equal(res.status, 400);
      const body = await res.json();
